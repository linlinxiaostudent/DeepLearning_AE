...
imgplot=plt.imshow(x_train[22,0,:,:,],cmap='gray')
plt.show()
...
model.add(Conv2D(n_filters, (filter_size1, filter_size1), batch_input_shape=(None, 1, img_width, img_height), padding="valid", activation="relu"))

model.add(MaxPooling2D(pool_size=(pool_size1, pool_size1)))

model.add(Conv2D(n_filters, (filter_size2, filter_size2), padding="valid", activation="relu"))
